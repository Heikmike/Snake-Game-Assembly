...
main:
	...
	main_loop:
		...
		addi sp, sp, -4
		stw ra, 0 (sp)
		; after get input, we should check if buttons are pressed
		add t5, zero, v0
		addi t6, zero, BUTTON_CHECKPOINT
		beq t5, t6, restore_section
		; hit test
		addi sp, sp, -4
   	 	stw ra, 0 (sp)
 		call hit_test
		...
		add t1, zero, v0
		add t3, zero, zero
		; hit test to check if we can continue move
		beq t1, t3, continue_move
		addi t3, zero, 2
		; hit test to check if we should end game
		beq t1, t3, end_game
		; hit test to check if we hit food, we should create food
		move_and_create_food:
			; move snake 
			; fisrt pass the return value of hit test to move_snake
			addi a0, zero, 1
			addi sp, sp, -4
			stw ra, 0 (sp)
			call move_snake
			...
			addi sp, sp, 4
    		ldw ra, 0 (sp)
			; check the return value of save_checkpoint
			add t7, zero, v0
			add t6, zero, zero
			beq t6, t7, end_save_section
			addi sp, sp, -4
			stw ra, 0 (sp)
			call blink_score
			addi sp, sp, 4
...
set_pixel:
	...
	jmpi set_cmp
	set_op:
    addi t1, zero, 1 ;-> why 1 shift left by x * 8 + y ???
    sll t1, t1, t0
...
move_snake:
	...
	snake_tail_move_part1:
		...
		; TODO I just don't know where v0 is fucking coming from.(20211118)
		; TODO I know it comes from the return value of hit test(20211125)
		addi t1, zero, 1
 		beq t0, t1, move_snake_end 
...
save_checkpoint:
	...
	addi t1, zero, 10
	cmp_10:
	blt t0, zero, finish_checkpoint_0
	beq t0, zero, finish_checkpoint_1
	addi t0, t0, -10
	jmpi cmp_10
	finish_checkpoint_0:
	addi t2, zero, 1
	stw t2, CP_VALID (zero)
...
restore_checkpoint:
	...
	addi t1, zero, 1
	beq t0, t1, end_restore_1
	beq t0, zero, end_restore_0
	; 1 when is valid
	end_restore_1:
	addi v0, zero, 1
	ret
...
wait:
	;only for test
	addi t5, zero, 5000 ; first counter 
	addi t6, zero, 5000 ; second counter
	jmpi loop_1 

	; we want to perform 1e8 iterations to wait 0.5s
	loop_1:
		beq t5, zero, loop_2
		addi t5, t5, -1
		jmpi loop_1
